...
# == Heatsink base (spreader plate) — defines the overall footprint ==
l_HS    = 30e-3          # heatsink length [m]
w_HS    = 20e-3          # heatsink width  [m]
t_HS    = 6e-3
k_HS    = 160

# == Die (40% of heatsink footprint, proportionally scaled) ==
# A_die = 0.40 * A_HS  ->  each dimension scaled by sqrt(0.40)
R_die   = 0.05
W_diss  = 100
l_die   = 10e-3
w_die   = 10e-3
...
# == Channel geometry ==
l_ch    = l_HS           # channel runs full heatsink length
h_ch    = 10e-3
w_ch    = 3e-3
w_wall  = 1e-3
...
A_die     = l_die * w_die          # die footprint [m2]
A_HS      = l_HS  * w_HS           # heatsink base footprint [m2]
coverage  = A_die / A_HS           # die-to-heatsink area ratio [-]

R_pcb   = t_pcb / (k_pcb * A_die)
R_HS    = t_HS  / (k_HS  * A_HS)  # 1D conduction over full heatsink base
...
print("-" * 80)
print(f"  Die coverage         =   {coverage*100:.1f} [%]")
